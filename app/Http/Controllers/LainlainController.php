...
class LainlainController extends Controller
{
    ...
    public function index(Request $request){
        app('App\Http\Controllers\HelperController')->automatically_delete_data();
         ...
         $current_right = $request->input('proses') ? $request->input('proses') : 1;
            return view('_officer.lain_lain.index',[
              // ================================== Count ============================
                'semua' => Pengaduan::where('tujuan_id' , 9)->count(),
              ...
              'pengaduan_masuk' => Pengaduan::where('status','Pengaduan Sedang Diverifikasi')
                                      ->where('tujuan_id',9)->orderBy('id','ASC')
        ...
        public function detail(Pengaduan $pengaduan, Request $request){
          return view('_officer.lain_lain.detail',[
            'title' => "Detail Lain-lain",
            'url' => $request->path(),
            'tujuan' => Tujuan::all(),
            'pengaduan' => $pengaduan,
    ...
    // proses disini sudah pasti selesai
      ...
      return redirect()->back()
        ...
        // ========================= Lain-lain Section Semua ====================
      public function section_semua(Request $request){
    
        $pagination = 10;
        $pengaduan = Pengaduan::where('tujuan_id' , 9)->orderBy('id','DESC');
        ...
        return view('_officer.lain_lain.semua',[
          'title' => 'Lain-lain Selesai',
          'url' => $request->path(),
          'pengaduan' => $pengaduan->paginate($pagination)->withQueryString(),
          'selesai' => Pengaduan::where('status','Pengaduan Selesai')->where('tujuan_id' , 9)->count(),
          'ditolak' => Pengaduan::where('status','Pengaduan Ditolak')->where('tujuan_id' , 9)->count(),
          'rating' => Pengaduan::whereNotNull(['rating', 'komentar'])->where('tujuan_id', 9)->count(),
          'semua' => Pengaduan::where('tujuan_id' , 9)->count(),
