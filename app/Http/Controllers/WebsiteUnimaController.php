...
class WebsiteUnimaController extends Controller
{
    public function index(Request $request){
      app('App\Http\Controllers\HelperController')->automatically_delete_data();
        ...
        $current_right = $request->input('proses') ? $request->input('proses') : 1;
           return view('_officer.website_unima.index',[
             // ================================== Count ============================
               'semua' => Pengaduan::where('tujuan_id' , 4)->count(),
             ...
             'pengaduan_masuk' => Pengaduan::where('status','Pengaduan Sedang Diverifikasi')
                                     ->where('tujuan_id','4')->orderBy('id','ASC')
    ...
    public function detail(Pengaduan $pengaduan, Request $request){
            return view('_officer.website_unima.detail',[
              'title' => "Detail Website Unima",
              'url' => $request->path(),
              'tujuan' => Tujuan::all(),
              'pengaduan' => $pengaduan,
...
public function update_store(Pengaduan $pengaduan, Request $request){
  ...
  // Setup Activity Log 
    ...
    if(auth()->user()->level == 'website_unima' || auth()->user()->can_website_unima ==1 || auth()->user()->level == 'petugas' ){
      $user = auth()->user()->id ;
  ...
  public function section_semua(Request $request){
    ...
    $pagination = 10;
    $pengaduan = Pengaduan::where('tujuan_id' , 4)->orderBy('id','DESC');
    ...
    return view('_officer.website_unima.semua',[
      'title' => 'Website Unima',
      'url' => $request->path(),
      'pengaduan' => $pengaduan->paginate($pagination)->withQueryString(),
      'selesai' => Pengaduan::where('status','Pengaduan Selesai')->where('tujuan_id' , 4)->count(),
      'ditolak' => Pengaduan::where('status','Pengaduan Ditolak')->where('tujuan_id' , 4)->count(),
      'rating' => Pengaduan::whereNotNull(['rating', 'komentar'])->where('tujuan_id', 4)->count(),
      'semua' => Pengaduan::where('tujuan_id' , 4)->count(),
