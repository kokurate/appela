...
class LearningManagementSystemController extends Controller
{
    public function index(Request $request){
      app('App\Http\Controllers\HelperController')->automatically_delete_data();
        ...
        $current_right = $request->input('proses') ? $request->input('proses') : 1;
           return view('_officer.lms.index',[
             // ================================== Count ============================
               'semua' => Pengaduan::where('tujuan_id' , 5)->count(),
             ...
             'pengaduan_masuk' => Pengaduan::where('status','Pengaduan Sedang Diverifikasi')
                                     ->where('tujuan_id','5')->orderBy('id','ASC')
    ...
    public function detail(Pengaduan $pengaduan, Request $request){
        return view('_officer.lms.detail',[
              'title' => "Detail Learning Management System",
              'url' => $request->path(),
              'tujuan' => Tujuan::all(),
              'pengaduan' => $pengaduan,
...
public function update_store(Pengaduan $pengaduan, Request $request){
  ...
  // Setup Activity Log 
    ...
    if(auth()->user()->level == 'lms' || auth()->user()->can_lms ==1 || auth()->user()->level == 'petugas' ){
      $user = auth()->user()->id ;
  ...
  public function section_semua(Request $request){
    ...
    $pagination = 10;
    $pengaduan = Pengaduan::where('tujuan_id' , 5)->orderBy('id','DESC');
    ...
    return view('_officer.lms.semua',[
      'title' => 'Learning Management System',
      'url' => $request->path(),
      'pengaduan' => $pengaduan->paginate($pagination)->withQueryString(),
      'selesai' => Pengaduan::where('status','Pengaduan Selesai')->where('tujuan_id' , 5)->count(),
      'ditolak' => Pengaduan::where('status','Pengaduan Ditolak')->where('tujuan_id' , 5)->count(),
      'rating' => Pengaduan::whereNotNull(['rating', 'komentar'])->where('tujuan_id', 5)->count(),
      'semua' => Pengaduan::where('tujuan_id' , 5)->count(),
